...
const Modal = ({ open, onOpenChange, children, className }: TModalProps) => {
    return (
        <Dialog.Root open={open} onOpenChange={onOpenChange}>
            <Dialog.Portal>
                <Dialog.Overlay className="fixed inset-0 z-40 bg-black bg-opacity-10 backdrop-blur-xs" />
...
const ModalBody = ({ children }: { children: ReactNode }) => {
    return <div className="p-4 md:p-5 space-y-4 text-gray-500 dark:text-gray-400">{children}</div>;
};
