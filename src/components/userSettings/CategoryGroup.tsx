...
const CategoryGroup = () => {
    const [selectedCategories, setSelectedCategories] = useState<TFeedCategories[]>(() => {
        return userPreference.get().categories;
    });

    const filteredCategories = useMemo(
        () => CATEGORIES_LIST.filter((cat) => ![TFeedCategories.ALL_NEWS, TFeedCategories.MY_FEED].includes(cat.id)),
        []
    );
